...
if (isset($_GET['action'])) {
	...
	if ($action=="hapus_lokasi") {
		$root->hapus_lokasi($_GET['id_lokasi']);
	}
	...
	if ($action=="tambah_kasir") {
		$root->tambah_kasir($_POST['nama'],$_POST['email'],$_POST['nama_kasir'],$_POST['password'],$_POST['id_lokasi'],$_POST['gambar']);
	}
	if ($action=="tambah_lokasi") {
		$root->tambah_lokasi($_POST['alamat']);
	}
	if ($action=="owner_tambah_kasir") {
	$root->owner_tambah_kasir($_POST['nama'],$_POST['email'],$_POST['nama_kasir'],$_POST['password'],$_POST['gambar'],$_POST['id_status'],$_POST['id_lokasi']);
	}
	if ($action=="hapus_user") {
		$root->hapus_user($_GET['id_user']);
	}
	if ($action=="edit_kasir") {
		$root->aksi_edit_kasir($_POST['nama'],$_POST['email'],$_POST['nama_kasir'],$_POST['password'],$_POST['gambar'],$_POST['id']);
	}
	if ($action=="owner_edit_kasir") {
		$root->owner_aksi_edit_kasir($_POST['nama'],$_POST['email'],$_POST['nama_kasir'],$_POST['password'],$_POST['alamat'],$_POST['status'],$_POST['gambar'],$_POST['id']);
	}
	if ($action=="edit_lokasi") {
		$root->aksi_edit_lokasi($_POST['alamat']);
	}
	if ($action=="edit_admin") {
		$root->aksi_edit_admin($_POST['nama'],$_POST['email'],$_POST['nama_kasir'],$_POST['password'],$_POST['gambar'],$_POST['id']);
	}
	if ($action=="edit_gambar") {
		$root->aksi_edit_gambar($_POST['nama'],$_POST['email'],$_POST['nama_kasir'],$_POST['password'],$_POST['gambar'],$_POST['id']);
	}
	...
	if ($action=="selesai_transaksi") {
		...
		$trx=date("d")."/AF/".$_SESSION['id']."/".date("y/h/i/s");
			...
			$query=$root->con->query("insert into transaksi set kode_kasir='$_SESSION[id]',total_bayar='$_POST[total_bayar]',no_invoice='$trx',id='$_POST[id]',id_lokasi='$_POST[id_lokasi]',tgl_transaksi='$_POST[tgl_transaksi]'");
