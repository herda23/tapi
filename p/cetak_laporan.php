...
class PDF extends FPDF
{
    function Header()
    {
        ...
        $this->cell(30,10,''.$_POST['tgl_laporan'].'');
    ...
    function data_barang(){
        ...
        if ($_POST['jenis_laporan']=="perhari") {
            ...
            $query=mysql_query("select transaksi.id_transaksi,transaksi.tgl_transaksi,transaksi.no_invoice,transaksi.total_bayar,transaksi.id,transaksi.id_lokasi,user.username from transaksi inner join user on transaksi.kode_kasir=user.id where transaksi.tgl_transaksi like '%$tanggal%' order by transaksi.id_transaksi desc");
        }else{
            $split1=explode('-',$tanggal);
