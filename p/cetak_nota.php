...
function LoadData(){
	...
	$data=mysql_query("select sub_transaksi.jumlah_beli,paket.nama_paket,paket.harga_jual,sub_transaksi.total_harga from sub_transaksi inner join paket on paket.id_paket=sub_transaksi.id_paket where sub_transaksi.id_transaksi='$id'");
...
function BasicTable($header, $data)
{
    ...
    foreach($data as $row)
    {
        ...
        $this->Cell(90,7,$row['nama_paket'],1);
...
$pdf->AliasNbPages();
$header = array('Qty', 'Nama Paket','Harga' ,'Total Harga');
$data = $pdf->LoadData();
$pdf->AddPage();
$pdf->Ln(20);
