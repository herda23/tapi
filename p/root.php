...
class penjualan
{
	...
	function tambah_kasir($nama,$email,$nama_kasir,$password,$id_lokasi,$gambar){
		$nama_kasir=str_replace(" ", "", $nama_kasir);
		$query=$this->con->query("select * from  user where username='$nama_kasir' and id_status='2'");
		if ($query->num_rows > 0) {
			$this->alert("Username  untuk Pegawai  sudah ada.");
			$this->go_back();
		}
		else{
			$password=sha1($password);
			$query2=$this->con->query("insert into user set nama='$nama',email='$email',username='$nama_kasir',password='$password',id_status='2',id_lokasi='$id_lokasi',gambar='$gambar'");
			if ($query2 ===  TRUE) {
				$this->alert("Data Pegawai berhasil dismpan");
				$this->redirect("users.php");
			}
			else{
				$this->alert("Pegawai Gagal Ditambahkan");
				$this->redirect("users.php");
			}
		}
	}
	...
	function tampil_setting(){
		$id_lokasi= $_SESSION['id_lokasi'];	
		$id= $_SESSION['id'];	
		$query=$this->con->query("select * from user where id='$id' and id_lokasi like '%$id_lokasi%'");
		$no=1;
		while ($data=$query->fetch_assoc()) {
			?>
			<tr>
	...
	function tampil_kasir_owner(){
		$query=$this->con->query("select * from user 
									...
									inner join level on user.id_status=level.id_status
	...
	function aksi_edit_kasir($nama,$email,$username,$password,$gambar,$id){
		if (empty($password)) {
			$query=$this->con->query("update user set nama='$nama',email='$email',username='$username',gambar='$gambar',date_created=date_created where id='$id'");
		}else{
			$password=sha1($password);
			$query=$this->con->query("update user set nama='$nama',email='$email',gambar='$gambar',date_created=date_created where id='$id'");
		}
	...
	function owner_aksi_edit_kasir($nama,$email,$username,$password,$id_lokasi,$id_status,$gambar,$id){
		if (empty($password)) {
			$query=$this->con->query("update user set nama='$nama',email='$email',username='$username',gambar='$gambar',id_lokasi='$id_lokasi',id_status='$id_status',date_created=date_created where id='$id'");
		}else{
			$password=sha1($password);
			$query=$this->con->query("update user set nama='$nama',email='$email',id_lokasi='$id_lokasi',gambar='$gambar',id_status='$id_status',date_created=date_created where id='$id'");
		}
	...
	function aksi_edit_admin($nama,$email,$username,$password,$gambar,$id){
		if (empty($password)) {
			$query=$this->con->query("update user set nama='$nama',email='$email',username='$username',gambar='$gambar',date_created=date_created where id='$id'");
		}else{
			$password=sha1($password);
			$query=$this->con->query("update user set nama='$nama',email='$email',username='$username',password='$password',gambar='$gambar',date_created=date_created where id='$id'");
		}

		if ($query === TRUE) {
			$this->alert("User berhasil di update,Silahkan Login Kembali");
			session_start();
			session_destroy();
		 	$this->redirect("users.php");
		}else{
	...
	function aksi_edit_gambar($nama,$email,$username,$password,$gambar,$id){
		if (empty($password)) {
			$query=$this->con->query("update user set nama='$nama',email='$email',username='$username',gambar='$gambar',date_created=date_created where id='$id'");
		}else{
			$password=sha1($password);
		...
		if ($query === TRUE) {
			$this->alert("Gambar berhasil di update,Silahkan Login Kembali");
			session_start();
			session_destroy();
		 	$this->redirect("users.php");
		}else{
			$this->alert("Gambar gagal di update");
		 	$this->redirect("users.php");
		}
	}
